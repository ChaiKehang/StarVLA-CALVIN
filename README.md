...
```bash
export PROJECT_ROOT=/path/to/StarVLA-CALVIN-E0
...
StarVLA 需要本项目的 dataset mixture 和 CALVIN websocket 兼容修改。因此完整复现
应使用上传后的 `ChaiKehang/starVLA` E0 分支，而不是只 clone 未修改的 upstream。
如何第一次创建该分支见本地
`plan/GitHub_仓库上传指导.md`。首次发布时必须先完成该指导的“仓库 A”；在 fork
和 `calvin-e0-abc-rel` 分支尚未推送前，下面的 clone 命令会失败，这是预期行为。

```bash
mkdir -p "$PROJECT_ROOT/code" "$PROJECT_ROOT/scripts/reference"

git clone --branch calvin-e0-abc-rel --single-branch \
  https://github.com/ChaiKehang/starVLA.git "$STARVLA_DIR"

git clone --recurse-submodules \
  https://github.com/mees/calvin.git "$CALVIN_DIR"
git -C "$CALVIN_DIR" checkout fa03f01f19c65920e18cf37398a9ce859274af76
git -C "$CALVIN_DIR" submodule update --init --recursive
...
```bash
git -C "$STARVLA_DIR" rev-parse HEAD
git -C "$CALVIN_DIR" rev-parse HEAD
git -C "$CALVIN_DIR/calvin_env" rev-parse HEAD
...
只使用个人 Conda 环境，不使用 `sudo`、系统 Python 或系统级 `pip`。
...
为保留实验命令，脚本仍传 `REPLAN_STEPS=5`。但当前 StarVLA CALVIN client 仅保存
该变量，没有用它控制刷新；实际按 policy server handshake 返回的
`action_chunk_size=8` 每 8 个环境 step 请求一个新 chunk。因此现有 ACL 3.214 的
有效重规划长度是 8，不能标成已验证的 replan=5。将来比较 5/8/10/12 前，必须先
修正 client 缓存逻辑并重新评测。
